...
const GroupCard = (props) => {
  const { gpName, _id, members, gpPhoto, memberRequest, type, allRequirement } =
    props.info;
  ...
  const isMember = members.filter(
    (user) => user.email === data.user.email
  ).length;
  const haveRequest = Boolean(
    memberRequest.filter((req) => req?.email === data.user?.email).length
  );
      ...
      >
        ...
        <div>
          ...
          <div className="flex items-center justify-between mt-3 ml-2">
            <div className="flex">
              {memberReverse.slice(0, 5).map((mem) => (
                <Avatar
                  key={mem.email}
            ...
            <div>
              {isMember ? (
                <Button
                  color="secondary"
                  variant="contained"
                  sx={{ background: "rgb(0 255 232 / 20%)", mt: 1 }}
                  component={NavLink}
